...
static int write_tree_level(IndexEntry *entries, int count, const char *prefix, ObjectID *id_out) {
    ...
    while (i < count) {
        ...
        if (slash) {
            // It's a subdirectory — collect all entries with same dir prefix
            char dirname[256];
            size_t dirlen = slash - rel;
            strncpy(dirname, rel, dirlen);
            dirname[dirlen] = '\0';
            ...
            if (prefix && strlen(prefix) > 0)
                snprintf(full_prefix, sizeof(full_prefix), "%s/%s", prefix, dirname);
            else
                snprintf(full_prefix, sizeof(full_prefix), "%s", dirname);

            while (j < count) {
                const char *p = entries[j].path;
                if (strncmp(p, full_prefix, strlen(full_prefix)) == 0 &&
                    (p[strlen(full_prefix)] == '/' || p[strlen(full_prefix)] == '\0'))
                    j++;
                else
                    break;
            }

            // Recurse for this subdir
            ObjectID sub_id;
